...
export default {
    name:'MyFooter',
    props:['todosListMyFooter','allCheckMyFooter','clearCheckedMyFooter'],
    computed:{
      doneTotal(){
        // return this.todosListMyFooter.reduce((pre,todoObj)=>{
        //   return pre + (todoObj.done ? 1:0)
        // },0)
        return this.todosListMyFooter.reduce((pre,todoObj) => pre + (todoObj.done ? 1:0),0)

      },
      todosTotal(){
        return this.todosListMyFooter.length
      },
      isAll:{
        get(){
